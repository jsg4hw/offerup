class ItemsController < ApplicationController
  ...
  def index
    ...
    @items = @q.result(:distinct => true).includes(:user, :category).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@items.where.not(:item_location_latitude => nil)) do |item, marker|
      ...
      marker.infowindow "<h5><a href='/items/#{item.id}'>#{item.title}</a></h5><small>#{item.item_location_formatted_address}</small>"
    end
  end
  ...
  def current_user_must_be_item_user
    ...
    def item_params
      params.require(:item).permit(:title, :price, :category_id, :user_id, :image, :item_location, :purchased)
    end
end
